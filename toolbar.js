...
(function(){
  ...
  css.textContent = `
    ...
    #_tb span{width:1px;height:14px;background:rgba(255,255,255,0.12);margin:0 2px;flex-shrink:0;}
  `;
  ...
  NAV_ITEMS.forEach((item, i) => {
    ...
    // active if current path starts with the href (handles /sms/ and /sms/index.html)
    const p = location.pathname;
    const h = item.href.replace(/\/$/, '');
    if (p === h || p === h + '/' || p.startsWith(h + '/') && h !== '/sms') {
      a.className = 'on';
    }
    nav.appendChild(a);
  });
